...
def nodelist(nodedict, targetsegment):
    ...
    for node in nodedict:
        ...
        nw = ipaddress.ip_network(unicode(targetsegment))
        nodeline = ''
        if ip in nw:
...
def autogenerate_network(data):
    ...
    for node in data["node"]:
        ...
        for network in data["network"]:
            nw = ipaddress.ip_network(unicode(network["address"]), strict=False)
            if ip in nw:
                generateflag = 1
        if generateflag == 0:
            adddict = {}
            ...
            adddict["address"] = str(ipaddress.ip_network(
                unicode(node["address"] + "/32")).supernet(new_prefix=int(defaultmask)))
